...
class Solution:
    ...
    def table_num(self, row: int, col: int) -> int:
        url = "http://32s465422x.yicp.fun/data/1sheet/sheet.html"
        start = requests.get(url)  # git方法获取网站信息
        start.encoding = "utf-8"  # 设置编码
        html = BeautifulSoup(start.text, "lxml")  # 通过BeautifulSoup进行解析, "lxml" "html.parser"

        hang = html.find_all("tr")[row]
        lie = hang.find_all("td")[col].string
